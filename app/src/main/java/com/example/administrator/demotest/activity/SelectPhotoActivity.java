...
import com.example.administrator.demotest.ZZImageLoader.AlbumBean;
import com.example.administrator.demotest.ZZImageLoader.AlbumListAdapter;
import com.example.administrator.demotest.ZZImageLoader.AlxBitmapUtils;
import com.example.administrator.demotest.ZZImageLoader.AlxPermissionHelper;
        ...
        switch (requestCode) {
            case SelectPhotoAdapter.REQ_CAMARA: {//1000如果是相机发来的
                ...
                if (RESULT_OK == resultCode) {//系统默认值
                    ...
                    Log.i("Alex", "拍摄图片暂存到了" + imageFilePath + "  角度是" + ZZImageLoder.readPictureDegree(imageFilePath));
                    //把图片压缩成指定宽高并且保存到本地
