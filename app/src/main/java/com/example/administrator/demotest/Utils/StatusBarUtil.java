...
import com.example.administrator.demotest.Utils.meizu.StatusbarColorUtils;
...
public class StatusBarUtil {
    ...
    public static void setColorDiff(Activity activity, @ColorInt int color) {
        ...
        transparentStatusBar(activity, Color.TRANSPARENT);
        ViewGroup contentView = (ViewGroup) activity.findViewById(android.R.id.content);
        // 移除半透明矩形,以免叠加
    ...
    public static void setTranslucent(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        ...
        setTransparent(activity, Color.TRANSPARENT);
    ...
    public static void setLightMode(Activity activity) {
        ...
        if (isXiaomi() && setMIUIStatusBarDarkIcon(activity, true)) {
            Log.e(TAG, "setLightMode xiaomi ok");
        } else if (isMeizu() && setMeizuStatusBarDarkIcon(activity, true)) {
            Log.e(TAG, "setLightMode meizu ok");
            return;
        } else if (isOppo() && setOppoStatusBarDarkIcon(activity, true)) {
            Log.e(TAG, "setLightMode oppo ok");
            return;
        }
    ...
    public static void setDarkMode(Activity activity) {
        ...
        if (isXiaomi() && setMIUIStatusBarDarkIcon(activity, false)) {
            Log.e(TAG, "setLightMode xiaomi ok");
        } else if (isMeizu() && setMeizuStatusBarDarkIcon(activity, false)) {
            Log.e(TAG, "setLightMode meizu ok");
            return;
        } else if (isOppo() && setOppoStatusBarDarkIcon(activity, false)) {
            Log.e(TAG, "setLightMode oppo ok");
            return;
        }
    ...
    private static boolean setMeizuStatusBarDarkIcon(@NonNull Activity activity, boolean darkIcon) {
        ...
        StatusbarColorUtils.setStatusBarDarkIcon(activity, darkIcon);
        return true;
    }
     ...
     */
    @TargetApi(Build.VERSION_CODES.KITKAT)
    private static void transparentStatusBar(Activity activity, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
    ...
    public static boolean isSupport() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && (isMeizu() || isXiaomi() || isOppo())) {
            return true;
    ...
    public static int getStatusBarHeight(Context context) {
        ...
        //刘海屏增加高度
        if (ScreenUtil.isConcaveScreens(context)) {
            if (Build.BRAND.contains("huawei")) {
                int[] ret = ScreenUtil.getNotchSize(context);
                int height = ret[1];
                if (height > result) {
                    result = height;
                }
            } else if (Build.BRAND.contains("oppo")) {
                String ret = ScreenUtil.getOPPONotchSize();
                Log.e("OPPONotchSize: ", ret);
            } else if (Build.BRAND.contains("vivo")) {
                // TODO: 2018/7/11  获取vivo凹形屏高度
            } else if (Build.BRAND.contains("xiaomi")) {
                result = ScreenUtil.getXiaomiNotchSize(context);
            }
        }
        return result;
    }
    ...
    public static boolean checkDeviceHasNavigationBar(Context context) {
        ...
        try {
            ...
            Log.e("checkDeviceHasNavigationBar", e.getMessage().toString());
        }
    ...
    private static boolean setOppoStatusBarDarkIcon(Activity activity, boolean lightStatusBar) {
        if (activity.getWindow() == null) {
            return false;
        }
    ...
    private static void setStatusBarIconDark(Activity activity, boolean dark) {
        try {
